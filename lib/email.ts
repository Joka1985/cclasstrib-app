...
function montarBotao(label: string, href: string, background: string) {
  return `<a href="${escapeHtml(href)}" style="display:inline-block;padding:12px 18px;border-radius:10px;background:${background};color:#ffffff;text-decoration:none;font-weight:700;font-size:14px;margin-right:10px;margin-bottom:10px;">${escapeHtml(label)}</a>`;
}
...
export async function enviarEmailSolicitacaoCliente({ para, nomeCliente, protocolo, resumo, anexoBase64, nomeArquivo }: { para: string; nomeCliente: string; protocolo: string; resumo?: ResumoSolicitacao; anexoBase64: string; nomeArquivo: string; }) {
  ...
  const { data, error } = await getResend().emails.send({
    ...
    html: montarLayoutEmail("Recebemos sua documentação", `<p>Olá, ${escapeHtml(nomeCliente)}</p><p>Seu envio foi recebido com sucesso.</p><p><strong>Protocolo:</strong> ${escapeHtml(protocolo)}</p>${montarResumoHtml(resumo)}<p>Em breve será enviado orçamento para aprovação.</p><p>Atenciosamente,<br />Equipe cClassTrib</p>`),
...
export async function enviarEmailRelatorioSuporte({ para, protocolo, loteId, cliente, documento, emailCliente, modoDocumentacao, resumo, anexoBase64, nomeArquivo }: { para: string; protocolo: string; loteId?: string; cliente: string; documento?: string; emailCliente?: string | null; modoDocumentacao?: string; resumo?: ResumoSolicitacao; anexoBase64: string; nomeArquivo: string; }) {
  ...
  const { data, error } = await getResend().emails.send({
    ...
    html: montarLayoutEmail("Relatório interno", `<p><strong>Protocolo:</strong> ${escapeHtml(protocolo)}</p>${loteId ? `<p><strong>Lote ID:</strong> ${escapeHtml(loteId)}</p>` : ""}<p><strong>Cliente:</strong> ${escapeHtml(cliente)}</p>${documento ? `<p><strong>Documento:</strong> ${escapeHtml(documento)}</p>` : ""}${emailCliente ? `<p><strong>E-mail do cliente:</strong> ${escapeHtml(emailCliente)}</p>` : ""}${modoDocumentacao ? `<p><strong>Modo de documentação:</strong> ${escapeHtml(modoDocumentacao)}</p>` : ""}${montarResumoHtml(resumo)}`),
    attachments: [{ filename: nomeArquivo, content: anexoBase64 }],
  });
  if (error) throw new Error(error.message);
  return data;
...
export async function enviarEmailOrcamentoCliente({ para, nomeCliente, protocolo, itensCobraveis, itensComRessalva, itensImprecisos, valorUnitario, valorTotal, observacao, observacaoOrcamento, tokenAcaoOrcamento }: { para: string; nomeCliente: string; protocolo: string; itensCobraveis?: number; itensComRessalva?: number; itensImprecisos?: number; valorUnitario?: string | number; valorTotal: string | number; observacao?: string | null; observacaoOrcamento?: string | null; tokenAcaoOrcamento: string; }) {
  checkConfig();
  const resend = getResend();
